...
// Tipo da função setTarefa
type SetTarefaType = (id: string, concluida:boolean) => void;
export type deleteTaksType = (id: string) => void;

// Definindo as props do componente Task
interface TaskProps extends TarefaType {
  setTarefa: SetTarefaType; // Adiciona a função setTarefa
  deleteTask:deleteTaksType;
}

export const Task: React.FC<TaskProps> = ({ id, titulo, concluida, setTarefa, deleteTask }) =>{

    const onClickStatus = () => {        
        setTarefa(id,concluida);
    }
    ...
    const handleImageStatus = () => {
        if(concluida){
           return  (<div className={styles.imageColumn}>
            <img src="../../public/checked.png" alt="Tarefa não concluída" onClick={onClickStatus} />
        </div>)    
        }else{
            return (<div className={styles.imageColumn}>
                <img src="../../public/check.png" alt="Tarefa concluída" onClick={onClickStatus} />
            </div>)     
        }
    }
